...
def centroids(seed,X,label=None):
    ...
    if label is not None:
        ...
        #print("unique labels are: ",cLabel)
        c = np.zeros((len(cLabel), X.shape[1]))
        cint = np.zeros((len(cLabel), X.shape[1]))
        for i,l in enumerate(cLabel):
            indexes = np.where(label==l)
            c[i,:] = bipolarize(rng,np.sum(X[indexes[0]],axis=0))
            #print(c[i,:])
            cint[i,:] = np.sum(X[indexes[0]],axis=0)
...
def merged_centroids(seed,X,label=None):
    rng = np.random.default_rng(seed)
    counts=[]
    if label is not None:
        cLabel = np.unique(label)
        ...
        for i,l in enumerate(cLabel):
            indexes = np.where(label==l)
            if math.floor(prev_label/100) == math.floor(label/100):
                c[activity_count,:] = bipolarize(rng,np.sum(X[indexes[0]],axis=0)) + c[activity_count,:]
            else:
                c[activity_count,:] = bipolarize(rng,np.sum(X[indexes[0]],axis=0))
...
def spatial_encode(seed,feat,im):
    ...
    for i,f in enumerate(feat):
        spatialHV[i] = bipolarize(rng,np.sum(f.reshape(-1,1)*im, axis=0)) # scale each unique channel hv by its real value and then sum across channels and then bipolarize.
    return spatialHV
...
def distances(v1, v2, metric='hamming'):
    '''
    This function expects 2D objects, ie. arrays of HVs.
    '''
...
def train_test_split_merged_accuracy(seed,XTrain,XTest,yTrain,yTest):
    rng = np.random.default_rng(seed)
    AM = centroids(rng,XTrain,label=yTrain)
    pred = classify(XTest,AM[1],'hamming',amLabels=AM[0])
    correct = 0
...
print(a)
print(b)
print(distances(h_a,h_b))
print(classify(h_a,h_b,"hamming"))